...
Filename: MainProgram-Hough.py
Description: Programm to run for use the algorithm with Hough-Transformation
"""
...
drive=DriveWay()
#initialise the class for the algorithm with hough transformation
houghLaneDetection = laneDetection()
#booblean varaible for finish the programm
running=True
#booblean varaible for delay for starting at the programm 
startGO=False
...
while running:
    ...
    if ret:
        ...
        if isGo==True:#drive the robot
            ...
            objPositionsOfLine = houghLaneDetection.houghLaneDetection(frame)
            ...
            if (not (objPositionsOfLine.getXPos1() is None) and not (objPositionsOfLine.getYPos1() is None) or not (objPositionsOfLine.getXPos2() is None) and not(objPositionsOfLine.getYPos2()is None)) and not(objPositionsOfLine.getAngle() is None):           
                #checks if the first time to drive after begin the program
                if(startGO==True):
                    time.sleep(2)
                    startGO=False
                #drive the robot with the contours algorithm 
                drive.doGo(objPositionsOfLine.getAngle())
